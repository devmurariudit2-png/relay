...
const reconcile = async (userId) => {
  ...
  const bank     = await Transaction.find({ user: userId, source: 'bank' });
  const internal = await Transaction.find({ user: userId, source: 'internal' });
  ...
  const pending = await Transaction.find({ user: userId, status: 'pending' });
  ...
  const stillPending = await Transaction.find({ user: userId, status: 'pending' });
  ...
  // ── 8. Build summary ──────────────────────────────────────────────────────
  const all     = await Transaction.find({ user: userId });
